...
class Runner:
    ...
    def __init__(self, base_results_dir: str = "./results", save: bool = True):
        ...
        # makes sure that the results are saved
        self.outdir = None
        if save:
            self.outdir = self.get_next_available_outdir(base_results_dir)

        # ---------------------------------------------------------------------
        # Defining parameters for VI sampler and training it
        # --------------------------------------------------------------------- 
        np.random.seed(2)
        ...
        def cosine_temper(step, *, total_steps, beta_min=0.01):
            t = step / total_steps
            return beta_min + 0.5 * (1 - beta_min) * (1 + jnp.cos(jnp.pi * t))

        steps = 10000
        temper_schedule = lambda s: cosine_temper(s, total_steps=steps, beta_min=0.4)


        # optimiser
        lr_sched  = optax.cosine_decay_schedule(0.08, steps)
        ...
        # ---------------------------------------------------------------------
        # Visual diagnostics of the results
        # --------------------------------------------------------------------- 
        # loss plot function
