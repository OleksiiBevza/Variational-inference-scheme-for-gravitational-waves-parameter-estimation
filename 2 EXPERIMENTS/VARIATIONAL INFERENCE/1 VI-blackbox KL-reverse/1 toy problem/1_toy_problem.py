#################################################################################
# This experiment implements a Block Neural Autoregressive Flow (BNAF) with a standard multivariate 
# normal base distribution 𝑁(0,𝐼𝑑) to approximate a multivariate Gaussian target. 
# The flow is trained with flow-based black-box variational inference, using a reparameterizable 
# normalizing flow and stochastic gradient optimization (Adam). Training minimizes the reverse KL(q||p)
#
# The structure of the code is:
# 1) Define prior bounds and likelihood
# 2) Construct the target density
# 3) Build the variational family (BNAF + bounding transforms)
# 4) Define the VI loss (reverse KL with optional tempering)
# 5) Optimize the flow parameters
# 6) Run the experiment
# 7) Results and diagnostics will be saved in an output directory
#################################################################################




# packages for normalizing flow
import jax
...
class Flow:
    ...
    # create 1D bijection mapping R into the interval specified by bounds
    @staticmethod
    ...
    # save bounders for each dimension into a single vector bijection
    def Bounder(self):
        return Stack(list(map(self.UnivariateBounder, self.bounds)))

    # enforce a specified parameter bounds, return a transformed distribution
    ...
    # build BNAF on standard normal base, apply bounding transforms to get variational distribution
...
class VI:
    ...
    # build a uniform prior distribution over per-dimension bounds.
    def get_prior(self, bounds):
        lo = jnp.array(bounds)[:, 0]
        hi = jnp.array(bounds)[:, 1]
        return Uniform(minval=lo, maxval=hi)

    # evaluate log-likelihood or return 0 otherwise
    def get_log_likelihood(self, likelihood=None, return_variance=False):
        ...
        if return_variance:
            # return log-likelihood and its variance the object.
        ...
        # return log-likelihood from the object.
    ...
    # compute Bayes factors, selection effects, and their variances for GW likelihood
    ...
    # run variational training of a flow-based posterior approximation
    # and return the trained flow
    def trainer(
        self,
        key,
        vmap=True,
        ...
        **tqdm_kwargs,
        ...
        # if no flow: build a default BNAF flow with bounds
        if flow is None:
            key, _key = jax.random.split(key)
            flow = Flow(bounds=bounds).default_flow(_key)  

        # split flow into trainable and untrainable parameters
        ...
        # define VI loss as the Monte Carlo estimate of KL(q||tempered target)
        ...
        # if no schedule then constant 1
...
##########################################################################
# MULTIVARIATE NORMAL LIKELIHOOD CLASS
##########################################################################
...
# packages for runner of the experiment
import numpy as np
import jax
from jax import random
import optax
...
class Runner:
    ...
    def __init__(
        self,
        dim=15,   # number of dimansions
        mean=None,
        cov=None,
        prior_bounds=None,
        steps=10000,     
        batch_size=2000,  # samples you draw from the variational distribution per training step to estimate the loss and gradient.
        learning_rate=0.065, # how big each parameter update is during optimization
        seed=0
    ):
        self.dim = dim
        self.mean = np.array([0.0, 2.0, 4.0, -5.0, -3.0, -1.0, 1.0, 5.0, 8.0, 10.0, 12.0, -7.0, -4.0, -11.0, -13.0])    # define means in each dimension
        self.seed = seed
    ...
    # cosne tempering function to account for variance
    ...
    #=========================DIAGNOSTICS====================================
    # extract variational inference samples
    ...
    # create corner plot (compare sampled vs target distributions)
    ...
    # compute mean and variance for sampled and target distributions
    def print_statistics(self, n_samples=10_000, outdir=None, filename="sample_statistics.txt"):
        ...
        # save results of descriptive statistics
        if outdir is not None:
